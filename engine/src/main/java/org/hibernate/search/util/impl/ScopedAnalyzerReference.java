...
/**
 * TODO: Probably here somewhere we need to add a method like AnalyzerReference getWrappedAnalyzer(String scope);
...
public final class ScopedAnalyzerReference implements AnalyzerReference {
	...
	private static ScopedAnalyzer createScopedAnalyzer(AnalyzerReference globalAnalyzer, Map<String, AnalyzerReference> analyzers) {
		ScopedAnalyzer analyzer = null;
		if ( globalAnalyzer.is( LuceneAnalyzerReference.class ) ) {
			analyzer = new ScopedAnalyzer( luceneAnalyzer( globalAnalyzer ) );
		}
		else {
			analyzer = new ScopedAnalyzer( null );
		}
