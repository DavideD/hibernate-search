...
/**
 * Execute the tests in {@link SearchNewEntityJmsMasterSlave} on the archives created in
 * {@link DeploymentJmsMasterSlaveWithInfinispan} adding the the dependencies as jars in the deployment.
 *
 * @author Davide D'Alto
 * @author Sanne Grinovero
 */
@RunWith(Arquillian.class)
public class SearchNewEntityJmsMasterSlaveWithInfinispanIT extends SearchNewEntityJmsMasterSlave {
	...
	private static class LibrariesLoader {
		...
		private static File[] init() {
			...
			File[] libraryFiles = resolver
					...
					.artifact( "org.hibernate:hibernate-search-infinispan:" + currentVersion )
					.resolveAsFiles();
			return libraryFiles;
		}
	}
	...
	public static Archive<?> createDeploymentMaster() throws Exception {
		WebArchive master = DeploymentJmsMasterSlaveWithInfinispan.createMaster( "infinispan-master" )
				.as( WebArchive.class );
		addLibraries( master );
		return master;
	}
	...
	public static Archive<?> createDeploymentSlave1() throws Exception {
		WebArchive slave = DeploymentJmsMasterSlaveWithInfinispan.createSlave( "infinispan-slave-1" )
				.as( WebArchive.class );
		addLibraries( slave );
		return slave;
	}
	...
	public static Archive<?> createDeploymentSlave2() throws Exception {
		WebArchive slave = DeploymentJmsMasterSlaveWithInfinispan.createSlave( "infinispan-slave-2" )
				.as( WebArchive.class );
		addLibraries( slave );
		return slave;
	}
