...
public class RoutingArbiter implements BackendQueueProcessor {
	...
	private final OwnerDefiningKey key;
	...
	public RoutingArbiter(InfinispanIndexManager infinispanIndexManager,
		...
		this.key = new OwnerDefiningKey( indexName );
	...
	private synchronized void failoverCheck(List<Address> newMembers) {
		if ( ! newMembers.contains( lastSeenMasterNode ) ) {
			...
			if ( selfElected ) {
				log.warn( "Self elected node " + localAddress + " ! Replaced " + lastSeenMasterNode );
				makeLocalMaster();
			}
			else {
				Address registeredMaster = (Address) channeledCache.get( key );
				log.warn( "Other node elected: " + registeredMaster + " ! Replaced " + lastSeenMasterNode );
