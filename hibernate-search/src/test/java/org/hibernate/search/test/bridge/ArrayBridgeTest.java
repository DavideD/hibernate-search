...
public class ArrayBridgeTest extends SearchTestCase {
	...
	public void testNullIndexing() throws Exception {
		Transaction tx = fullTextSession.beginTransaction();
		{
			List<ArrayBridgeTestEntity> results = findResults( fullTextSession, "embedded", null );
			...
			assertEquals( "Wrong result returned from an indexed array", notIndexable.getName(), results.get( 0 )
					.getName() );
		}
		tx.commit();
	}
	...
	public void testNumericNullIndexing() throws Exception {
		Transaction tx = fullTextSession.beginTransaction();
		{
			List<ArrayBridgeTestEntity> results = findResults( fullTextSession, "embeddedNum", null );
