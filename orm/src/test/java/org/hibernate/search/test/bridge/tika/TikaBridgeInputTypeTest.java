...
public class TikaBridgeInputTypeTest extends SearchTestBase {
	...
	public void testDefaultTikaBridgeWithListOfString() throws Exception {
		try ( Session session = openSession() ) {
			...
			persistBook( session, book );

			indexBook( session );
			searchBook( session, "contentAsListOfString" );

			List<Book> result = search( session, "contentAsListOfString", "Tika" );
			assertEquals( "there should be a match", 1, result.size() );
		}
	}
